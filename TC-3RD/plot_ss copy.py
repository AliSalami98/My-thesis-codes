...
# Load data
csv_path = r'C:\Users\ali.salame\Desktop\plots\Thesis figs\TC_slow\Exergy\Edest_results_pcharged.csv'
...
plt.ylabel(r'Heater heat transfer rate $\dot{Q}_\text{heater}$ [kW]', fontsize=14)
...
plt.xlabel(r'Pressure ratio $r_\text{p}$ [-]', fontsize=14)
plt.ylabel(r'Cooler heat transfer rate $\dot{Q}_\text{cooler}$ [kW]', fontsize=14)
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
# plt.grid(True)
plt.tight_layout()
...
# plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(save_dir, "Ex_eff.eps"), format='eps')
