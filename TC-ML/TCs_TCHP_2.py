...
counter = 0
with open(r'C:\Users\ali.salame\Desktop\Thermodynamics\data files\ML correlations\comps 3.csv') as csv_file:    
...
for j in range(L):
    ...
    test1 = np.array([[data['p2_in [pa]'][j], data['p2_out [pa]'][j], data['omega2 [rpm]'][j], data['Theater2 [K]'][j], data['Tw_in [K]'][j], data['T2_in [K]'][j]]])  # shape (1, 6)
    # test1 = np.array([[data['p2_out [pa]'][j]/data['p2_in [pa]'][j], np.sqrt(data['p2_out [pa]'][j]*data['p2_in [pa]'][j]), data['Theater2 [K]'][j], data['Tw_in [K]'][j],data['omega2 [rpm]'][j], data['T2_in [K]'][j]]])  # shape (1, 6)
...
PR = np.array(data['p2_out [pa]']) / np.array(data['p2_in [pa]'])
...
if APPLY_CALIBRATION:
    ...
    ytrue_mdot = np.array(data["mdot [kg/s]"])
    ytrue_pcl  = np.array(data["Pcooling2 [W]"])
    ytrue_tout = np.array(data["T2_out [K]"])
...
mdot_true_sorted = np.array(data['mdot [kg/s]'])[sorted_indices]
mdot_pred_sorted = a_mdot[sorted_indices]

Pcool_true_sorted = np.array(data['Pcooling2 [W]'])[sorted_indices]
Pcool_pred_sorted = a_Pcool[sorted_indices]

Tout_true_sorted = np.array(data['T2_out [K]'])[sorted_indices]
...
ytrue_mdot = np.asarray(data["mdot [kg/s]"]) * 1e3        # kg/s
ytrue_pcl  = np.asarray(data["Pcooling2 [W]"]) * 1e-3       # W
ytrue_tout = np.asarray(data["T2_out [K]"]) - 273.15            # K
...
yhat_mdot = np.asarray(a_mdot) * 1e3       # kg/s
...
plt.scatter(ytrue_mdot, yhat_mdot, s=50, edgecolor="red", facecolor="lightgrey", label="Simulated", linewidths=2.0)
...
plt.plot(yt_sorted, 0.9*yt_sorted, "--", label="±10% error", c="b")
plt.plot(yt_sorted, 1.1*yt_sorted, "--", c="b")
plt.scatter(ytrue_pcl, yhat_pcl, s=50, edgecolor="red", facecolor="lightgrey", label="Simulated", linewidths=2.0)
...
plt.scatter(ytrue_tout, yhat_tout, s=50, edgecolor="red", facecolor="lightgrey", label="Simulated", linewidths=2.0)
