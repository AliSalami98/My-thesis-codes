...
hidden_size = 16 # LSTM hidden layer size
...
plt.savefig(r"C:\Users\ali.salame\Desktop\plots\Thesis figs\TCHP_data\control\LSTM_training.eps", format='eps', bbox_inches='tight')
...
# ------------------------
torch.save(model.state_dict(), "LSTM_Johan3/lstm_model.pth")
joblib.dump(scaler_input, "LSTM_Johan3/scaler_input.pkl")
joblib.dump(scaler_output, "LSTM_Johan3/scaler_output.pkl")
...
ax.plot(y_pred_C,
           label=f"LSTM Prediction (MSE: {mse_Tw_out:.2f} K², R²: {r2_Tw_out:.3f})",
           linestyle="dashed", color="orange", linewidth=2)
ax.set_xlabel("Time [s]", fontsize=14)
ax.set_ylabel("$T_{w, out}$ [°C]", fontsize=14)
...
model = LSTMCellModel(input_size, hidden_size, output_size, num_layers)
model.load_state_dict(torch.load("LSTM_Johan3/lstm_model.pth"))
model.eval()

scaler_input = joblib.load("LSTM_Johan3/scaler_input.pkl")
scaler_output = joblib.load("LSTM_Johan3/scaler_output.pkl")
...
for cycle in range(cycles):
    ...
    ax.plot(
        ...
        label=r"LSTM input $T_\text{w, sup}$" if cycle == 0 else "", zorder=3
    )
    ...
    ax.plot(
        ...
        label=rf"LSTM predicted $T_{{\mathrm{{w,\,sup}}}}$ (MSE: {mse_Tw_out:.2f} $\mathrm{{K}}^2$, R$^2$: {r2_Tw_out:.3f})" if cycle == 0 else "", zorder=2
    )
...
plt.savefig(r"C:\Users\ali.salame\Desktop\plots\Thesis figs\TCHP_data\control\LSTM_validation.eps", format='eps', bbox_inches='tight')
