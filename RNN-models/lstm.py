...
prediction_length = 40  # Number of steps to predict ahead
batch_size = 32
...
# Initialize LSTM model, loss, optimizer, scheduler
model = LSTMModel(input_size, hidden_size, output_size, prediction_length, num_layers, dropout)
...
# ------------------------
torch.save(model.state_dict(), "LSTM2/lstm_model.pth")
joblib.dump(scaler_input, "LSTM2/scaler_input.pkl")
joblib.dump(scaler_output, "LSTM2/scaler_output.pkl")
...
ax[0].plot(test_predictions_denorm[:, 0],
           label=f"LSTM Prediction (MSE: {mse_pc:.2f} Bar², R²: {r2_pc:.3f})",
...
model = LSTMModel(input_size, hidden_size, output_size, prediction_length, num_layers, dropout)
model.load_state_dict(torch.load("LSTM2/lstm_model.pth"))
model.eval()

scaler_input = joblib.load("LSTM2/scaler_input.pkl")
scaler_output = joblib.load("LSTM2/scaler_output.pkl")
# Recursion

# Parameters
prediction_length = 500  # Steps to predict per cycle
total_steps = 40 * (sequence_length + prediction_length)  # Total simulation length
