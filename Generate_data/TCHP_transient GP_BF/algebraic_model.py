...
def compute_cycle_inputs(
    *,
    ...
    f_hpv = 8e-9 * hpev #min(max(4.765e-9 * hpev + 6.3625e-8, 1e-7), 4.5e-7) #
    f_lpv = 3e-9 * min(lpev, 60) #min(max(6.023e-9 * lpev - 1e-7, 0.2e-7), 3e-7) #
    ...
    mc_in_dot = 0.6 * 1e-3 * model3_1_LR.predict(a3_T)[0]
    ...
    if config.hft > config.hft_v:
        mft_l_dot = 1e-6
        mft_v_dot = 1e-6 * (((hpev - lpev) + 88) / (2 * 88)) * np.sign(config.pft - config.pbuff1) * np.sqrt(2 * config.Dft * np.abs(config.pft - config.pbuff1))
    else:
        mft_l_dot = f_lpv * np.sign(config.pft - config.pe) * np.sqrt(2 * config.Dft_l * np.abs(config.pft - config.pe))
        mft_v_dot = 1e-6 * (((hpev - lpev) + 88) / (2 * 88)) * np.sign(config.pft - config.pbuff1) * np.sqrt(2 * config.Dft_v * np.abs(config.pft - config.pbuff1))
    ...
    mtc2_dot = 0.6 * 1e-3 * model2_1_LR.predict(a2_T)[0] #y_pred_real_GP2[0] #
    Ttc2_out = model2_2_LR.predict(a2_T)[0] #y_pred_real_GP2[2] #

    me_in_dot = mft_l_dot

    Pcooler2_pred = 0.8 * model2_3_LR.predict(a2_T)[0] #y_pred_real_GP2[1] #
    Pcooler3_pred = 0.8 * model3_3_LR.predict(a3_T)[0]
