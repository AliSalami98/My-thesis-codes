...
with open(r'C:\Users\ali.salame\Desktop\Thermodynamics\data files\ML correlations\comps 2.csv') as csv_file:    
    ...
    for row in csv_reader:
        ...
        data['Tfume [K]'].append(float(row['Tfume [°C]']) + 273.15)
        ...
        data['mw_dot [kg/s]'].append(float(row['mw_dot [l/min]'])/60)
...
Tfume = np.array(data['Tfume [K]'])

Tw = np.array(data['Tw_in [K]'])
T1_in = np.array(data['T1_in [K]'])
T2_in = np.array(data['T2_in [K]'])
mw_dot = np.array(data['mw_dot [kg/s]']) * 0.3

X1_array = np.array([Pr1, Theater1, Tw, w1])
X2_array = np.array([Pr2, Theater2, Tw, w2])
X3_array = np.array([Pr3, Theater2, Tw, w3])
...
y1_1 = np.array(data['mdot1 [kg/s]']) * 1e3
y1_2 = np.array(data['T1_out [K]'])
y1_3 = np.array(data['Pcooling1 [W]'])

y2_1 = np.array(data['mdot [kg/s]']) * 1e3
