...
def post_process(
    ...
    for k in range(len(t)):
        ...
        # Trwo_target = (a_Pbuff1[-1] + Pcooler1_pred + Pcooler2_pred + Pcooler3_pred)/(config.cpw * mw1_dot) + Tw_in
        # config.Trwo = Trwo_target #config.Trwo + 1/50 * (Trwo_target - config.Trwo)
        # Tw_rec_in = (mw2_dot * config.Tc_w[0] + mw1_dot * config.Trwo)/mw_dot
        # mg_dot = 18.125 * mCH4_dot
        
        # Pfhx = (-3.59 * mw_dot - 624.78 * mg_dot + 0.128 * Tw_rec_in - 33.54) * (Tfume_in - Tw_rec_in) #  (2005.8 * mg_dot - 3.87 * mw_dot + 0.083) * (Tfume_in - Tw_rec_in) #(477.74 * mg_dot + 1.25) * (Tfume_in - Tw_rec_in)

        # Pheat_out_target = a_Pgc[-1] + a_Pbuff1[-1] + Pcooler1_pred + Pcooler2_pred + Pcooler3_pred + Pfhx
        # config.Pheating_out = Pheat_out_target #config.Pheating_out + 1/20 * (Pheat_out_target - config.Pheating_out)
