...
def init_flash_tank(x, pft_in, pe):
    config.pft = min(max(x[3 * (config.Nc + config.Ne) + 13], 20e5), 70e5) #config.pbuff1 #
    ...
    config.hft = min(max(x[3 * (config.Nc + config.Ne) + 14], config.hft_l + 20e3), config.hft_v - 20e3) #config.hihx1 
...
def init_flash_tank_post(y, k, pft_in, pe):
    config.pft = min(max(y[3 * (config.Nc + config.Ne) + 13, k], 20e5), 70e5) #cconfig.pbuff1 #
    state = get_state(CP.PQ_INPUTS, config.pft, 0)
    config.muft_l = state.viscosity()
    config.hft_l = state.hmass()
    config.pft_l = state.p()
    ...
    config.pft_v = state.p()
    config.Dft_v = state.rhomass()
    config.hft = min(max(y[3 * (config.Nc + config.Ne) + 14, k], config.hft_l + 20e3), config.hft_v - 20e3) #config.hihx1 #
