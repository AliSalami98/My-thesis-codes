...
def combined_state_space_model(
    ...
    xbuff2 = np.hstack((config.pbuff2, config.hbuff2, config.Tbuff2_wall))
    ...
    Zbuff2 = np.array([[z11, z12, 0], [z21, z22, 0], [0, 0, z33]])

    f1 = config.Qbuff2_conv + mbuff2_in_dot * (hbuff2_in - xbuff2[1])
    f2 = mbuff2_in_dot - mbuff2_out_dot
    f3 = -(config.Qbuff2_w_conv + config.Qbuff2_conv)
    fbuff2_vec = np.array([f1, f2, f3])

    Zbuff2_inv = np.linalg.inv(Zbuff2)
    dxbuff2dt = np.dot(Zbuff2_inv, fbuff2_vec)

    config.dpbuff2dt = dxbuff2dt[0]
    config.dhbuff2dt = dxbuff2dt[1]
    config.dTbuff2_walldt = dxbuff2dt[2]
