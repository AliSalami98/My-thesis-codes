...
def objective(control_inputs):
    Nc, Np, U_current, a_Tw_out_sp_MPC, mw_dot, Tw_in, a_J, Theater1_current, U1_current, D1_current, A1_coeffs, B1_coeffs, D1_coeffs, poly_U1 = params
    ...
    X_next = predict_next_state_cell(model, scaler_input, scaler_output, U_current, future_controls, sequence_length, prediction_length)  # shape: (20, 2)
    ...
    for i in range(prediction_length):
        ...
        Theater1_mpc.append(Theater1_current[0][0])
        U1_current = np.column_stack([omegab_opt_moves[0]])

    Theater1_mpc = np.array(Theater1_mpc)
    Theater1_mpc_norm = np.maximum(Theater1_mpc - 1073.15, 0) / 15

    # RNN
    alpha = 1     # Tracking Tw_out
    beta = 0 #1e-4     # Penalize omegab usage
    gamma = 0.1  # Penalize Δpc
    delta = 0.1 # Penalize Δomegab
    sanka = 1e-3

    # # LSTM
    # alpha = 1     # Tracking Tw_out
    # beta = 1e-4     # Penalize omegab usage
    ...
    COP_mpc = omegab_norm/(4186 * 0.023 * T_norm)
    # print(normalized_Delta_Hpev )
    # Final cost calculation
    cost = (
        alpha * np.sum(T_norm ** 2) +
        beta * np.sum(COP_mpc ** 2) +
...
def run_mpc(bounds, initial_guess):
    Nc, Np, U_current, a_Tw_out_sp_MPC, mw_dot, Tw_in, a_J, Theater1_current, U1_current, D1_current, A1_coeffs, B1_coeffs, D1_coeffs, poly_U1 = params
...
#     Nc = 1
#     model = load_RNN(model_path, input_size, hidden_size, output_size, num_layers)
...
if __name__ == "__main__":
    ...
    Np = prediction_length
    Nc = 1
    model = load_LSTM(model_path, input_size, hidden_size, output_size, num_layers)
...
mpc_times = []   # add before: for kt in range(total_time):
...
for kt in range(total_time):
    ...
    params = (Nc, Np, U_current, a_Tw_out_sp_MPC, mw_dot, Tw_in, a_J, Theater1_current, U1_current, D1_current, A1_coeffs, B1_coeffs, D1_coeffs, poly_U1)
    # if kt > sequence_length:
    t0 = time.perf_counter()
    omegab_opt, Hpev_opt = run_mpc(bounds, initial_guess)

    elapsed = time.perf_counter() - t0
    print(f"[Iteration {kt}] MPC optimization time: {elapsed*1000:.2f} ms")
    # print(pc_opt)
    # Build future control sequence (shape: [20, 4])
    future_controls = np.column_stack([
        np.full(prediction_length, omegab_opt),
    ...
    algebraic_out = compute_cycle_inputs(
        dt=dt,
        hpev=Hpev_opt, lpev=Lpev_opt,
        Tw_in=Tw_in, Theater1=Theater1, Theater2=Theater2,
        omega1=omega1, omega2=omega2_opt, omega3=omega3, omegab=omegab_opt,
    ...
    initial_guess = np.concatenate([np.full(Nc, omegab_opt), np.full(Nc, Hpev)])

    U1_current = np.column_stack([omegab_opt])
    D1_current = np.column_stack([omega1])
    U2_current = np.column_stack([omegab_opt])
    D2_current = np.column_stack([omega2_opt])
    
    U_new = np.array([Tw_out_real, omegab_opt, Hpev, mw_dot, Tw_in])
    ...
    Tw_out_ss.append(Tw_out_real)  # Record the last output of the current step
    ...
    pc_rnn_ss.append(pc_rnn)
    Tw_out_rnn_ss.append(Tw_out_rnn)
