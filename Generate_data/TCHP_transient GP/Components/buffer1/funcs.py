...
def init_buffer1(x, pbuff1_in, pe):
    config.pbuff1 = min(max(x[3 * (config.Nc + config.Ne) + 7], 20e5), 60e5) # #pbuff1_in #
...
def calc_buffer1(mbuff1_in_dot, mbuff1_out_dot, Dbuff1_in, mw1_dot_1, Tw_in, me_out_dot):
    config.vbuff1_n = (me_out_dot) / (config.Dbuff1 * config.ACO2_orifice)
...
def init_buffer1_post(y, k, pbuff1_in, pe):
    config.pbuff1 = min(max(y[3 * (config.Nc + config.Ne) + 7,  k], 20e5), 60e5) # pbuff1_in #
