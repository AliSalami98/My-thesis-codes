...
from Components.buffer2.funcs import(
    init_buffer2,
    calc_buffer2,
    model_buffer2,
...
from Components.FHX import fhx_predict_Q
import config
import torch
import numpy as np
...
class LSTMCellModel(nn.Module):
    ...
    def forward(self, past_inputs, future_controls):
        ...
        c = [torch.zeros(batch_size, self.hidden_size, device=past_inputs.device) for _ in range(self.num_layers)]
        ...
        # Stack predictions: (batch, pred_len, 2)
...
class LSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, pred_length, num_layers, dropout):
        ...
        # Output size is now output_size * pred_length to predict multiple steps
        self.fc = nn.Linear(hidden_size, output_size * pred_length)
    
    def forward(self, x):
        # x shape: (batch, seq_length, input_size)
        out, _ = self.lstm(x)  # LSTM returns (batch, seq_length, hidden_size)
        out = self.fc(out[:, -1, :])  # Use last time step, shape: (batch, output_size * pred_length)
...
def load_LSTM(model_path, input_size, hidden_size, output_size, num_layers):
    model = LSTMCellModel(input_size, hidden_size, output_size, num_layers)
    model.load_state_dict(torch.load(model_path))
    model.eval()  # Set the model to evaluation mode
    return model

def load_RNN(model_path, input_size, hidden_size, output_size, num_layers):
    model = RNNCellModel(input_size, hidden_size, output_size, num_layers)
    model.load_state_dict(torch.load(model_path))
    model.eval()  # Set the model to evaluation mode
    return model

# Define a function for inference that correctly handles sequences
def predict_next_state(model, scaler_input, scaler_output, current_sequence, sequence_length):
    # Ensure current_sequence is a 2D array (sequence_length, input_size)
    current_sequence = np.array(current_sequence)
    updated_sequence = current_sequence[-sequence_length:]
    input_data_scaled = scaler_input.transform(updated_sequence)
    input_tensor = torch.tensor(input_data_scaled, dtype=torch.float32).unsqueeze(0)
    
    model.eval()
    with torch.no_grad():
        next_states_scaled = model(input_tensor)  # Shape: (1, pred_length, output_size)
    
    next_states_scaled = next_states_scaled.squeeze(0).numpy()  # Shape: (pred_length, output_size)
    next_states = scaler_output.inverse_transform(next_states_scaled)
    
    return next_states

def predict_next_state_cell(model, scaler_input, scaler_output, current_sequence, future_controls,
                                sequence_length, prediction_length):
    # Keep only the last `sequence_length` steps
    current_sequence = np.array(current_sequence)[-sequence_length:]  # shape: (seq_len, 6)
    future_controls = np.array(future_controls)[:prediction_length]   # shape: (pred_len, 4)
    
    # Scale
    input_scaled = scaler_input.transform(current_sequence)
    control_scaled = scaler_input.transform(
        np.hstack([np.zeros((prediction_length, 1)), future_controls])
    )[:, 1:]  # Only keep control columns (4)

    # Convert to tensors
    input_tensor = torch.tensor(input_scaled[np.newaxis, :, :], dtype=torch.float32)
    control_tensor = torch.tensor(control_scaled[np.newaxis, :, :], dtype=torch.float32)

    # Predict
    model.eval()
    with torch.no_grad():
        output_scaled = model(input_tensor, control_tensor).squeeze(0).numpy()  # (pred_len, output_size)
    
    # Inverse transform
    output_real = scaler_output.inverse_transform(output_scaled)
    return output_real
...
def combined_state_space_model(
    ...
    # f1 = config.Qbuff1_conv + me_out_dot * (hbuff1_in - xbuff1[1])
    ...
    xbuff1 = np.hstack((config.hbuff1, config.Tbuff1_wall))

    z11 = (config.Dbuff1) * config.Vbuff1
    z22 = (config.mbuff1_wall * config.c_cu)
    Zbuff1 = np.array([[z11, 0], [0, z22]])

    f1 = config.Qbuff1_conv + me_out_dot * (hbuff1_in - xbuff1[0])
    f2 = -(config.Qbuff1_w_conv + config.Qbuff1_conv)
    fbuff1_vec = np.array([f1, f2])

    Zbuff1_inv = np.linalg.inv(Zbuff1)
    dxbuff1dt = np.dot(Zbuff1_inv, fbuff1_vec)

    config.dhbuff1dt = dxbuff1dt[0]
    config.dTbuff1_walldt = dxbuff1dt[1]

    xbuff2 = np.hstack((config.hbuff2, config.Tbuff2_wall))

    z11 = (config.Dbuff2) * config.Vbuff2
    z22 = (config.mbuff2_wall * config.c_cu)
    Zbuff2 = np.array([[z11, 0], [0, z22]])

    f1 = config.Qbuff2_conv + me_out_dot * (hbuff2_in - xbuff2[0])
    f2 = -(config.Qbuff2_w_conv + config.Qbuff2_conv)
    fbuff2_vec = np.array([f1, f2])

    Zbuff2_inv = np.linalg.inv(Zbuff2)
    dxbuff2dt = np.dot(Zbuff2_inv, fbuff2_vec)

    config.dhbuff2dt = dxbuff2dt[0]
    config.dTbuff2_walldt = dxbuff2dt[1]


    z21 = config.dDdp_hbuff2 * config.Vbuff2
    f2 = mbuff2_in_dot - mbuff2_out_dot
    config.dpbuff2dt = 1/z21 * (f2)
