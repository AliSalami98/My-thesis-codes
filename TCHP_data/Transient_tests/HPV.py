...
d_Pheat_out = np.asarray(d_Pheat_out[500:N_target])          # [W]
d_Pevap     = np.asarray(d_Pevap[500:N_target])              # [W]
d_COP       = np.asarray(d_COP[500:N_target])                # [-]
d_Tw_out       = np.asarray(d_Tw_out[500:N_target])                # [-]
d_Tc_out       = np.asarray(d_Tc_out[500:N_target])                # [-]
d_SH       = np.asarray(d_SH[500:N_target])                # [-]
d_pe       = np.asarray(d_pe[500:N_target])                # [-]
d_pc       = np.asarray(d_pc[500:N_target])                # [-]
d_Lpev       = np.asarray(d_Lpev[500:N_target])                # [-]
d_Hpev       = np.asarray(d_Hpev[500:N_target])                # [-]
d_omegab       = np.asarray(d_omegab[500:N_target])                # [-]


# Time vectors in minutes
t1_ss = np.arange(len(d_Pheat_out)) * (Ts_exp/60.0)   # experimental time base [min]

# ========= HPV LOOP (controls high-side pressure) =========
fig_hpv, (ax_hpv1, ax_hpv2, ax_hpv3) = plt.subplots(3, 1, figsize=(10, 7), sharex=True)

# --- High-side pressure (primary output) ---
# Optional sim overlay:
# ax_hpv1.plot(t_ss, pc_ss, '--', lw=1.8, label='Sim: High-side', color='#006400')
ax_hpv1.plot(t1_ss, d_pc, '-', lw=2.3, label='High-side', color='#006400')
ax_hpv1.set_ylabel(r"Pressure [bar]", fontsize=14)
ax_hpv1.legend(fontsize=11)
ax_hpv1.grid(True, alpha=0.3)
...
# --- HPV input (manipulated variable) ---
ax_hpv3.plot(t1_ss, d_Hpev, lw=2.3, label='HPV', color='#483D8B')
ax_hpv3.set_xlabel("Time [min]", fontsize=14)
ax_hpv3.set_ylabel(r"Valve opening [%]", fontsize=14)
ax_hpv3.legend(fontsize=11)
ax_hpv3.grid(True, alpha=0.3)

# --- Increase tick label size only ---
for ax in (ax_hpv1, ax_hpv2, ax_hpv3):
    ax.tick_params(axis='both', which='major', labelsize=14)

fig_hpv.tight_layout()
plt.savefig(r"C:\Users\ali.salame\Desktop\plots\Thesis figs\TCHP_data\transient\HPV.eps", format='eps', bbox_inches='tight')
