...
d_Pheat_out = np.asarray(d_Pheat_out[:N_target])          # [W]
d_Pevap     = np.asarray(d_Pevap[:N_target])              # [W]
d_COP       = np.asarray(d_COP[:N_target])                # [-]
d_Tw_out       = np.asarray(d_Tw_out[:N_target])                # [-]
d_Tc_out       = np.asarray(d_Tc_out[:N_target])                # [-]
d_SH       = np.asarray(d_SH[:N_target])                # [-]
d_pe       = np.asarray(d_pe[:N_target])                # [-]
d_pc       = np.asarray(d_pc[:N_target])                # [-]
d_Lpev       = np.asarray(d_Lpev[:N_target])                # [-]
d_Hpev       = np.asarray(d_Hpev[:N_target])                # [-]
d_omegab       = np.asarray(d_omegab[:N_target])                # [-]
d_Theater1       = np.asarray(d_Theater1[:N_target])                # [-]
d_Theater2       = np.asarray(d_Theater2[:N_target])                # [-]


# Time vectors in minutes
t1_ss = np.arange(len(d_Pheat_out)) * (Ts_exp/60.0)   # experimental time base [min]

# ========= BURNER FAN ω_bf (drives heating capacity) =========
fig_bf, (ax_bf1, ax_bf2, ax_bf3) = plt.subplots(3, 1, figsize=(10, 7), sharex=True)

# Heating & Evaporation Power (primary + related)
# ax_bf1.plot(t1_ss, d_Pheat_out/1000.0,  '-', lw=2.3, label='Total recovered', color='#FF4500')
# ax_bf1.plot(t1_ss, d_Pevap/1000.0,      '-', lw=2.3, label='Evaporator',     color='#00CED1')
# ax_bf1.plot(t1_ss, d_Tw_out, '-', lw=2.3, label= 'Supply water', color="#B84E44")
ax_bf1.plot(t1_ss, d_pc, '-', lw=2.3, label='High-side', color='#006400')
ax_bf1.plot(t1_ss, d_pe, '-', lw=2.3, label='Low-side', color='#3CB371')
ax_bf1.set_ylabel("Pressure [bar]", fontsize=14)
ax_bf1.legend(fontsize=11)
ax_bf1.grid(True, alpha=0.3)

# # COP (performance index)
# ax_bf2.plot(t1_ss, d_COP,  '-', lw=2.3, color='#008000')
# ax_bf2.set_ylabel("Thermal COP [-]", fontsize=14)
# ax_bf2.legend(fontsize=11)
# ax_bf2.grid(True, alpha=0.3)

ax_bf2.plot(t1_ss, d_Theater1, '-', lw=2.3, label= 'Heater1', color='#8A2BE2')
ax_bf2.plot(t1_ss, d_Theater2, '-', lw=2.3, label= 'Heater2', color='#DA70D6')
ax_bf2.set_ylabel("Temperature [°C]", fontsize=14)
ax_bf2.legend(fontsize=11)
ax_bf2.grid(True, alpha=0.3)

# ω_bf input
ax_bf3.plot(t1_ss, d_omegab, lw=2.3, label='Burner fan', color='#FFD700')
ax_bf3.set_xlabel("Time [min]", fontsize=14)
ax_bf3.set_ylabel(r"Speed [rpm]", fontsize=14)
ax_bf3.legend(fontsize=11)
ax_bf3.grid(True, alpha=0.3)

# --- Increase tick label size only ---
for ax in (ax_bf1, ax_bf2, ax_bf3):
    ax.tick_params(axis='both', which='major', labelsize=14)
fig_bf.tight_layout()

plt.savefig(r"C:\Users\ali.salame\Desktop\plots\Thesis figs\TCHP_data\transient\omegab.eps", format='eps', bbox_inches='tight')
